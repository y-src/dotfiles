...
-- Required before plugins are imported.
vim.g.mapleader = ' '
...
-- [[ Configure plugins ]]
-- NOTE: `opts = {}` is equivalent to `require('module').setup({})`
require('lazy').setup({
  -- Git related plugins
  'tpope/vim-fugitive',
  'tpope/vim-rhubarb',
...

  {
    ...
    dependencies = {
      -- Install LSPs to neovim stdpath
      ...
      -- Neovim docs and signature help
      'folke/neodev.nvim',
    },
  },
...
-- NOTE: Ensure terminal emulator supports 24-bit color in the TUI.
...
-- Get the git root directory based on the current buffer's path
...
-- Broadcast nvim-cmp, for code completion intergration with LSP
...
cmp.setup {
  enabled = function()
    -- Enable cmp on nvim-dap buffers
...
-- Setup cmp for REPL in nvim-dap-ui
